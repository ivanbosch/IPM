...
class myPDF extends FPDF {
  ...
  function myHeader($db){
    ...
    $this->Cell(270, 6, "(DOMESTIC - By advisor)", 0, 1, "C");
  }
  ...
  function advisorSales($db) {
    ...
    $result = $db->query("SELECT staff_ID AS ID, COUNT(sales_ID) AS Amount FROM sales WHERE sales_Type = 'Domestic' GROUP BY staff_ID");
    $y = 80;
    $i = 0;
    $amount = 0;
    //SELECT staff_ID AS ID, currency_ID AS c_ID, currency_Rate AS Rate, sales_Charge AS Charge FROM sales WHERE sales_Type = 'Domestic'
    while ($row = $result->fetch_assoc()) {
      $id = $row['ID'];
      $this->setXY(10, $y);
      $this->Cell(10, 10, "", 0, 0);
  ...
  function fareAmount($db) {
    ...
    $result = $db->query("SELECT staff_ID AS ID, currency_ID AS c_ID, currency_Rate AS Rate, sales_Charge AS Charge FROM sales WHERE sales_Type = 'Domestic'");
    while ($row = $result->fetch_assoc()) {
    ...
    foreach ($advisors as $advisor) {
      $this->setXY(58, $y);
      $totalLocal += $advisor;
      $totalUSD += $advisor/$fetchRow['currency_Rate'];
    ...
    $this->setXY(40, $y+10);
    $this->Cell(18, 10, "Fare USD:", 1, 0, 'C');
    $this->Cell(22, 10, number_format($totalUSD), 1, 0, 'C');
    if ($this->getTableEnd() < $y) {
      $this->setTableEnd($y);
    }
  }
  ...
  function cash($db) {
    $this->SetFont('Times', '', 10);
    $currency = $this->getCurrencyVar();
    $result = $db->query("SELECT staff_ID AS ID, sales_Charge AS Charge , payment_Type AS Type, currency_ID AS c_ID FROM sales WHERE sales_Type = 'Domestic' AND payment_Type='Cash'");
    $y = 80;
    $total = 0;
    $advisors = array();
    while ($row = $result->fetch_assoc()) {
    ...
    if ($this->getTableEnd() < $y) { $this->setTableEnd($y); }
  }

  function card($db) {
      $this->SetFont('Times', '', 10);
      $currency = $this->getCurrencyVar();
      $result = $db->query("SELECT staff_ID AS ID, sales_Charge AS Charge, currency_Rate AS Rate, payment_Type AS Type, currency_ID AS c_ID FROM sales WHERE sales_Type = 'Domestic' AND payment_Type='Card'");
      $y = 80;
      $x = 0;
      $total = array(3);
      $advisors = array();
  ...
  function TotalPaid($db) {
    $this->SetFont('Times', '', 10);
    $currency = $this->getCurrencyVar();
    $result = $db->query("SELECT staff_ID AS ID, sales_Charge AS Charge , payment_Type AS Type, currency_ID AS c_ID FROM sales WHERE sales_Type = 'Domestic'");
    $y = 80;
    $total = 0;
    $advisors = array();
    while ($row = $result->fetch_assoc()) {
  ...
  function commissions($db) {
    ...
    $result = $db->query("SELECT staff_ID AS ID, sales_Charge AS Charge, commission_Rate AS Rate, currency_ID AS c_ID FROM sales WHERE sales_Type = 'Domestic'");
