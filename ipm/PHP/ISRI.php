...
class myPDF extends FPDF {
  ...
  function myHeader($db){
    ...
    $this->Cell(270, 6, "(INTERLINE USD Rate = ".$row['currency_Rate'].")", 0, 1, "C");
    $id = $this->getID();
  ...
  function originalBlanks($db) {
    ...
    $result = $db->query("SELECT blanks.blank_Type AS blanks FROM blanks
                          ...
                          WHERE sales.staff_ID = '$id' AND sales.sales_Type='Interline' AND (sales.currency_ID='1' OR sales.currency_ID='$currency')");
    $y = 90;
    while ($row = $result->fetch_assoc()) {
      $blanks = $row['blanks'];
      $this->setXY(10, $y);
  ...
  function fareAmount($db) {
    ...
    $result = $db->query("SELECT sales.sales_Charge as Charge, sales.currency_ID as ID, sales.currency_Rate as Rate FROM sales INNER JOIN currency
                          ON currency.currency_ID = sales.currency_ID AND sales.sales_Type = 'Interline' AND sales.staff_ID ='$id' AND (sales.currency_ID = '1' OR sales.currency_ID = '$currency')");
    $y = 90;
    while ($row = $result->fetch_assoc()) {
      $this->setXY(35, $y);
      if ($row['ID'] == 1) { //Was paid in dollars get the current conversion rate
        $fetchRate = $db->query("SELECT currency_Rate from currency WHERE currency_ID = $currency");
        $fetchRow = $fetchRate->fetch_assoc();
        $this->Cell(10, 10, $row['Charge'], 1, 0, 'C');
        $this->Cell(17.5,10, number_format($fetchRow['currency_Rate'], 4), 1, 0,'C');
        $this->Cell(17.5, 10, number_format($row['Charge']*$fetchRow['currency_Rate'], 2), 1, 0, 'C');
        $totalUSD += $row['Charge'];
        $totalLocal += $row['Charge']*$fetchRow['currency_Rate'];
      } else { //Was paid in local currency
        $this->Cell(10, 10, number_format($row['Charge']/$row['Rate'], 2), 1, 0, 'C');
        $this->Cell(17.5,10, $row['Rate'], 1, 0,'C');
        $this->Cell(17.5, 10, $row['Charge'], 1, 0, 'C');
        $totalLocal += $row['Charge'];
        $totalUSD += $row['Charge']/$row['Rate'];
      }
      $y += 10;
    }
    $this->setXY(35, $y);
  ...
  function cash($db) {
    ...
    $currency = $this->getCurrencyVar();
    $id = $this->getID();
    $result = $db->query("SELECT sales_Charge AS Charge , payment_Type AS Type, currency_ID AS ID FROM sales WHERE sales_Type = 'Interline' AND staff_ID='$id' AND (currency_ID = '1' OR currency_ID = '$currency')");
    $y = 90;
    $total = 0;
    while ($row = $result->fetch_assoc()) {
      $this->setXY(80, $y);
      if ($row['Type'] != 'Cash') {
        ...
        if ($row['ID'] == 1) { //Was paid in dollars get the current conversion rate
          $fetchRate = $db->query("SELECT currency_Rate from currency WHERE currency_ID = $currency");
          $fetchRow = $fetchRate->fetch_assoc();
          $this->Cell(20, 10, number_format($row['Charge']*$fetchRow['currency_Rate'], 2), 1, 0, 'C');
          $total+=$row['Charge']*$fetchRow['currency_Rate'];
        } else { //Was paid in local currency
  ...
  function card($db) {
    ...
    $currency = $this->getCurrencyVar();
    $id = $this->getID();
    $result = $db->query("SELECT sales_Charge AS Charge , payment_Type AS Type, currency_ID AS ID, card_Digits AS Digits, currency_Rate AS Rate FROM sales WHERE sales_Type = 'Interline' AND staff_ID='$id' AND (currency_ID = '1' OR currency_ID = '$currency')");
    $y = 90;
    $totalUSD = 0;
    $totalLocal = 0;
    while ($row = $result->fetch_assoc()) {
      ...
      if ($row['Type'] != 'Card') {
        ...
        if ($row['ID'] == 1) { //Was paid in dollars get the current conversion rate
          $fetchRate = $db->query("SELECT currency_Rate from currency WHERE currency_ID = $currency");
          $fetchRow = $fetchRate->fetch_assoc();
          $this->Cell(20, 10, number_format($row['Charge'], 2), 1, 0, 'C');
          $this->Cell(20, 10, number_format($row['Charge']*$fetchRow['currency_Rate'], 2), 1, 0, 'C');
          $totalUSD += $row['Charge'];
          $totalLocal += $row['Charge']*$fetchRow['currency_Rate'];
        } else { //Was paid in local currency
          $this->Cell(20, 10, number_format($row['Charge']/$row['Rate'], 2), 1, 0, 'C');
          $this->Cell(20, 10, $row['Charge'], 1, 0, 'C');
          $totalUSD += $row['Charge']/$row['Rate'];
          $totalLocal += $row['Charge'];
        }
  ...
  function commissions($db) {
    ...
    $id = $this->getID();
    $result = $db->query("SELECT sales_Charge AS Charge, commission_Rate, currency_ID AS ID FROM sales WHERE sales_Type = 'Interline' AND staff_ID='$id' AND (currency_ID = '1' OR currency_ID = '$currency')");
    ...
    while ($row = $result->fetch_assoc()) {
      $this->setY($y);
      if ($row['ID'] == 1) {
        $x = $db->query("SELECT currency_Rate FROM currency WHERE currency_ID = '$currency'");
        $x1 = $x->fetch_assoc();
        $amount = $row['Charge']*$x1['currency_Rate'];
      } else {
        $amount = $row['Charge'];
      }
  ...
  function whiteSpace($db) {
    ...
    for ($i = 90; $i < $this->getTableEnd(); $i+=10) {
      ...
      $this->Cell(20, 10, "", 1, 0, 'C');
      $this->Cell(20, 10, "", 1, 0);
      $result = $db->query("SELECT * FROM commissions WHERE commission_Type = 'Interline'");
      $x = 90/mysqli_num_rows($result);
      while ($row = $result->fetch_assoc()) {
  ...
  function totals($db) {
    ...
    $result = $db->query("SELECT COUNT(sales_ID) AS blanks FROM sales
                          WHERE staff_ID = '$id' AND sales_Type='Interline'");
    $row = $result->fetch_assoc();
